%% this script extracts the diffusion tensor from the diffusion image and saves it

% fill in the following fields to set the script. Each path must contain
% the path to a file
diffusion_image = 'r_dti.nii';  % file containing the image to process
bvals = 'bvals';  % files containing the gradient information
bvecs = 'bvecs';
dti_tensor = 'dti_tensor.mat';  % name of the file for saving

% extract and save tensor
dti = extract_tensor(diffusion_tensor, bvals, bvecs);
save(dti_tensor, 'dti')
